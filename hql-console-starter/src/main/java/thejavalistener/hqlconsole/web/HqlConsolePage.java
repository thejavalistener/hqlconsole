...
public final class HqlConsolePage
{
	...
	private static String TEMPLATE_PARTE_1 =
		...
		<div id="split">
		  ...
		  <div id="panel-resultado" style="position:relative">
		    <div id="error-panel" hidden style="position:absolute;inset:0;z-index:10;box-sizing:border-box;overflow:scroll;padding:16px;background:var(--fondo,#fff)"><div id="error-panel-msg"></div><button type="button" id="error-panel-close" style="margin-top:12px">Cerrar</button><pre id="error-panel-detail" style="white-space:pre"></pre></div>
		...
		/** Ubica el menÃº junto al nombre mÃ¡s largo, no al borde ancho del panel. */
		function posicionHorizontalDelMenu(boton) {
		  const caja = boton.getBoundingClientRect();
		  const medidor = document.createElement('canvas').getContext('2d');
		  medidor.font = getComputedStyle(boton).font;
		  let ancho = 0;
		  listaEntidades.querySelectorAll('.entidad-item').forEach(function(item) {
		    ancho = Math.max(ancho, medidor.measureText(item.textContent).width);
		  });
		  return Math.min(caja.right, caja.left + Math.ceil(ancho) + 16);
		}
	...
	private static String TEMPLATE_PARTE_2 =
		...
		estiloIntegrado.textContent = 'body{padding:0}.barra{justify-content:flex-start;align-items:center;flex-wrap:nowrap;width:100%;min-height:58px;background:#fff;border:0;border-bottom:1px solid var(--borde);border-radius:0;padding:12px 20px;gap:8px}.barra h1{display:flex;align-items:center;gap:5px;white-space:nowrap;font-size:17px}.icono-consola{font-size:21px;line-height:1;color:var(--acento)}.separador-titulo{opacity:.45;font-weight:400}.barra #idioma-titulo{box-sizing:border-box;width:3em;text-align:center;padding:1px 6px;border-radius:4px;background:#dce8ff;color:#174f96}.barra #atajo-ejecutar{margin-left:8px;display:inline-flex;align-items:center;min-height:26px;padding:4px 9px;border-radius:4px;background:#2f6fbd;color:#fff;font-size:12px;font-weight:600;white-space:nowrap}#split,#error{margin-left:12px;margin-right:12px}#panel-entidades{margin-right:0;border:1px solid var(--borde);border-top:0;border-right:0;border-radius:0 0 0 6px;background:#eef4ff}#panel-editor{border:1px solid var(--borde);border-top:0;border-left:0;border-radius:0 0 6px 0;background:#fff;padding:0;gap:0}#hql,#sql{border:0;border-radius:0;background:transparent;outline:0}#hql:focus,#sql:focus{outline:0}#entidades-cabecera{background:transparent}#toggle-entidades{opacity:.55}#toggle-entidades:hover{opacity:1}.marca-mapeada{margin-left:5px;color:#1769aa;font-weight:700}#panel-entidades.contraido .filtros-tablas{display:none}body.modo-hql #panel-entidades.contraido,body.modo-sql #panel-entidades.contraido{width:30px}#split.entidades-contraidas #solapas{width:calc(var(--ancho-editor) + 38px)}@media(max-width:720px){.barra{padding:10px 12px}.barra #atajo-ejecutar{margin-left:auto}#panel-entidades{border:1px solid var(--borde);border-radius:6px}#panel-editor{border:1px solid var(--borde);border-radius:6px}}';
